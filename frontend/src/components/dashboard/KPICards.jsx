  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
      ...
      <div className="bg-white rounded-lg shadow p-5 border-l-4 border-green-500">
        <p className="text-sm text-gray-500 font-medium">Vendas no Mes</p>
        <p className="text-2xl font-bold mt-1 text-gray-900">{formatBRL(salesValue)}</p>
        <p className="text-xs text-gray-400 mt-1">{periodSales?.totalInvoices || 0} notas emitidas</p>
      </div>

      {/* Meta do Mes */}
      <div className={`bg-white rounded-lg shadow p-5 border-l-4 ${metaAtingida ? 'border-green-500' : 'border-yellow-500'}`}>
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500 font-medium">Meta do Mes</p>
          {metaAtingida && <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">ATINGIDA</span>}
        </div>
        <p className="text-2xl font-bold mt-1 text-gray-900">{formatBRL(META)}</p>
        <div className="mt-2">
      ...
      {/* Super Meta */}
      <div className={`bg-white rounded-lg shadow p-5 border-l-4 ${superMetaAtingida ? 'border-purple-500' : 'border-gray-300'}`}>
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500 font-medium">Super Meta</p>
          {superMetaAtingida && <span className="text-xs font-bold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full">ATINGIDA</span>}
        </div>
        <p className="text-2xl font-bold mt-1 text-gray-900">{formatBRL(SUPER_META)}</p>
        <div className="mt-2">
          <div className="flex items-center justify-between text-xs mb-1">
